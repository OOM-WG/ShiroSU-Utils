	...
	<div class="music-player" @click="handleContainerClick">
		...
		<div v-if="!isExpanded" class="music-player-collapsed">
			<div class="collapsed-content">
				...
				<!-- 右侧：播放按钮 -->
				<n-button circle type="primary" @click.stop="togglePlay" class="main-play-btn">
					<template #icon>
						<n-icon size="18">
			...
			<div class="player-content" @click.stop>
				...
				<div class="controls-row">
					...
					<div class="volume-section">
						<n-button circle size="small" @click="toggleMute" class="volume-btn-expanded">
							<template #icon>
								<n-icon>
				...
				<div class="playlist-section">
					...
					<div class="playlist-container">
						<div
							...
							:key="index"
							:class="['playlist-item', {active: index === currentTrackIndex}]"
...
const saveState = () => {
	try {
		...
		localStorage.setItem(STORAGE_KEYS.CURRENT_TRACK, currentTrackIndex.value.toString())
		localStorage.setItem(STORAGE_KEYS.IS_MUTED, isMuted.value.toString())
		localStorage.setItem(STORAGE_KEYS.CURRENT_TIME, currentTime.value.toString())
	} catch (error) {
...
// 当前播放的歌曲
const currentTrack = computed(() => playlist[currentTrackIndex.value] || playlist[0])
...
const previousTrack = () => {
	currentTrackIndex.value =
		currentTrackIndex.value > 0 ? currentTrackIndex.value - 1 : playlist.length - 1
...
const loadTrack = () => {
	...
	audioElement.value.addEventListener(
		'loadedmetadata',
		() => {
			if (currentTime.value > 0 && currentTime.value < audioElement.value!.duration) {
...
.music-player-collapsed {
	background: v-bind('`rgba(255, 255, 255, ${config.theme.backgroundOpacity})`');
...
.music-player-collapsed:hover {
	...
	background: v-bind('`rgba(255, 255, 255, ${Math.min(config.theme.backgroundOpacity + 0.03, 1)})`');
}
...
@media (prefers-color-scheme: dark) {
	...
	.music-player-collapsed:hover {
		background: v-bind('`rgba(24, 24, 27, ${Math.min(config.theme.backgroundOpacity + 0.03, 1)})`');
	}
