...
const avatarUrl = computed(() => {
	return props.member.avatarPic ?? `https://q1.qlogo.cn/g?b=qq&nk=${props.member.qq}&s=640`
})
</script>
		...
		<div class="member-card">
			<div class="member-content">
				...
				<div class="data">
					...
					<div class="profiles">
						<div v-if="member.projects && member.projects.length > 0" class="info-row">
							<div class="info-icon">
								<VTIconCode class="desc-icon code" />
							</div>
							<div class="info-content">
								<span v-for="(project, index) in member.projects" :key="project.label">
									<VTLink :href="project.url" :no-icon="true" class="desc-link">
										{{ project.label }}
									</VTLink>
									<span v-if="index < member.projects.length - 1" class="separator">•</span>
								</span>
							</div>
						</div>

						<div v-if="member.location && member.location.trim()" class="info-row">
							<div class="info-icon">
						...
						<div v-if="member.languages && member.languages.length > 0" class="info-row">
							<div class="info-icon">
								<VTIconGlobe class="desc-icon" />
							</div>
							<div class="info-content">
								<span v-for="(language, index) in member.languages" :key="language">
									{{ language }}
									<span v-if="index < member.languages.length - 1" class="separator">•</span>
								</span>
							</div>
						</div>

						<div v-if="member.website && member.website.url && member.website.label" class="info-row">
							<div class="info-icon">
