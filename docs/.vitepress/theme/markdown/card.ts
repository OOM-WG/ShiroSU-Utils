...
import {entries, fromEntries, isPlainObject, isString, stringifyProp} from '../utils/utils.js'
...
const CARD_PROPS = ['title', 'desc', 'logo', 'link', 'color', 'hoverShadow', 'shadow', 'theme']

const checkCardProps = (config: unknown): CardOptions | null => {
	if (isPlainObject(config) && isString(config['title']))
		return fromEntries(
			entries(config).filter(
				(item): item is [string, string] => CARD_PROPS.includes(item[0]) && isString(item[1])
			)
...
`
	...
	md.renderer.rules.fence = (...args): string => {
		...
		if (realInfo === 'card') return cardRender(tokens, index, options, <MarkdownEnv>env)
